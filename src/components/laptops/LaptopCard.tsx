...
export function LaptopCard({ laptop }: LaptopCardProps) {
  ...
  const handleOrderViaWhatsApp = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!user) {
      ...
      toast({
        ...
        description: "Please log in to your account to place a WhatsApp order.",
        variant: "destructive",
      });
      router.push('/login');
    }
  ...
  const whatsappMessage = `Hello! I'm interested in ordering the ${laptop.name} (ID: ${laptop.id}). Price: ${formattedPrice}. Please confirm availability and design.`;
  ...
  const handleAddToCart = () => {
    ...
    toast({
      title: "Added to Cart",
      description: `${laptop.name} is in your shopping list.`,
    });
  };
        ...
        <div className="relative p-2">
          ...
          <Link href={`/laptops/${laptop.id}`} className="block aspect-square relative overflow-hidden rounded-[1.5rem] bg-gray-50">
            <Image
              ...
              priority={laptop.id === 'laptop-32' || laptop.id === 'laptop-31'} 
            />
          ...
          <div className="mt-auto space-y-4">
            ...
            <div className="flex flex-col gap-2">
              <Button
                asChild
                size="lg"
                className="w-full h-14 rounded-2xl bg-green-600 hover:bg-green-700 text-white font-black uppercase tracking-[0.2em] text-[10px] shadow-lg shadow-green-100 transition-all"
              >
                <a 
                  href={whatsappLink} 
                  target="_blank" 
              ...
              <Button 
                variant="ghost"
                size="sm"
                className="h-10 rounded-xl font-black uppercase tracking-widest text-[9px] text-gray-400 hover:text-primary"
                onClick={handleAddToCart}
                disabled={laptop.stock === 0}
              >
                <ShoppingCart className="w-3.5 h-3.5 mr-2"/>
                {laptop.stock === 0 ? 'Out of Stock' : 'Add to Shopping List'}
              </Button>
            </div>
          </div>
        </CardContent>
